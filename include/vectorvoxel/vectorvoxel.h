...
{
typedef VECTORVOXEL_EXPORT std::function<void(int x0, int y0, int height, int color)> OnData;
...
class VECTORVOXEL_EXPORT Screen
{
  public:
    ...
    constexpr void MoveBy(float x_perc, float y_perc, float z_perc, double angle_perc, float pitch_perc);
    constexpr void ResetCamera();
    constexpr void Move(float x, float y, float z, double angle, float pitch);

    void Render(const std::vector<std::vector<int>> &height_map, const std::vector<std::vector<int>> &color_map,
                OnData when_line);

    void RenderSync(const std::vector<std::vector<int>> &height_map, const std::vector<std::vector<int>> &color_map,
                    OnData when_line);
  ...
  private:
    ...
    void CastRay(int screen_x, double angle, const std::vector<std::vector<int>> &height_map,
