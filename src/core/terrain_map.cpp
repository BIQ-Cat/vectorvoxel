...
void TerrainMap::setZIndex(std::shared_ptr<TerrainOverlay> overlay,
  ...
  int end_x = std::max(overlay->get_x() + overlay->get_width(), width);
  int end_y = std::max(overlay->get_y() + overlay->get_height(), height);
  ...
  if (start_x < end_x && start_y < end_y) {
    ...
    for (auto other : overlaps) {
      if (!other->goesBefore(old_z_index, overlay->id))
        other->markDirtyArea(0, 0, overlay->get_width(), overlay->get_height());
    }
  }
}
